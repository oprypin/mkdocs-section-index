...
**[Plugin][] for [ProperDocs][] to allow clickable sections that lead to an index page**
...
With this `nav` in *properdocs.yml* (or without `nav` but with [an equivalent directory structure](https://github.com/oprypin/mkdocs-section-index/tree/master/example/docs)):
...
The *borgs/index.md* page is merged as the index of the "Borgs" section. Normally sections in [ProperDocs][] cannot be clickable as pages themselves, but this plugin makes that possible.
...
* [readthedocs](https://properdocs.org/user-guide/styling-your-docs/#readthedocs)
* [nature](https://github.com/waylan/mkdocs-nature)

[themes]: https://properdocs.org/user-guide/choosing-your-theme/

## Usage notes

The kind of *nav* as shown above also happens to be what ProperDocs produces when `nav` is omitted; it detects [`index.md` and `README.md`][nav-gen] pages and automatically puts them as the first item.
...
Normally in ProperDocs [`nav`][nav], the items can be one of:
...
[nav]: https://properdocs.org/user-guide/custom-themes/#nav
[Section]: https://properdocs.org/user-guide/custom-themes/#section
[Page]: https://properdocs.org/user-guide/custom-themes/#page
[children]: https://github.com/ProperDocs/properdocs/blob/2781efbce10e68f1b119465072ed1c52710e9ef2/properdocs/structure/pages.py#L42
[Link]: https://properdocs.org/user-guide/custom-themes/#link
...
* This is too controversial to be enabled by default, or even be part of ProperDocs at all. This has been [discussed in the past and dropped](https://github.com/mkdocs/mkdocs/pull/1042#issuecomment-260813540). The main reason is that in ProperDocs there's no requirement for a *nav*'s structure to follow the actual directory structure of the doc files. Consequently, there's no natural way to deduce that a document should become the index page of a section just from its location, even if it's named *index.md*. Although if the *nav* is [omitted & generated][nav-gen], then yes, such an assumption works. It also works in the vast majority of actual usages *with* a *nav*, but that doesn't help.
...
[nav-gen]: https://properdocs.org/user-guide/writing-your-docs/#configure-pages-and-navigation
